class DurationChart {
    ...
    updateChart() {
        ...
        // Bin durations
        const maxDuration = d3.max(vis.data, d => d.duration);
        const durationBins = d3.range(0, maxDuration + 10, 1000).map(duration => ({
            duration: duration,
            count: vis.data.filter(d => d.duration >= duration && d.duration < duration + 60).length
        }));
        ...
        const y = d3.scaleLinear()
            .domain([0, d3.max(durationBins, d => d.count)])
            .nice()
            .range([vis.height, 0]);
        ...
        path.on("mousemove", (event) => {
            ...
            const closest = durationBins.reduce((prev, curr) =>
                Math.abs(x(curr.duration) - mouseX) < Math.abs(x(prev.duration) - mouseX) ? curr : prev
            );
  
            vis.hoverCircle
                .style("display", "block")
                .attr("cx", x(closest.duration))
                .attr("cy", y(closest.count));
  
            vis.tooltip
                .style("display", "block")
                .html(`Duration: ${closest.duration.toFixed(1)} sec<br>Count: ${closest.count}`)
